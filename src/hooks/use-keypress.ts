...
export function useKeypress({
	...
	const onKeyDown = useCallback(
		(ev: Event) => {
			...
			const inFormField =
				IGNORE_FOCUS.has(el?.tagName ?? "") || Boolean(el?.isContentEditable);
			// The upstream hook bails out of EVERY chord while focus is in a field.
			// That's right for a bare key — "k" must type a k, not open a palette —
			// but wrong for a modifier chord: ⌘K is expected to work from anywhere,
			// and this app has a form field on nearly every page, so the shortcut
			// was dead exactly where it's most useful. A chord carrying a modifier
			// can't collide with typing, so it fires regardless of focus.
			const everyChordHasModifier = normalized.every((chord) =>
				chord.split("+").some(isMod),
			);
			if (inFormField && !everyChordHasModifier) {
				return;
			}
