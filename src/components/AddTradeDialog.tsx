...
export function AddTradeDialog({ accounts, trade, onClose }: { accounts: Account[]; trade?: Trade; onClose: () => void }) {
  ...
  return (
    ...
    >
      ...
      >
        <h3 style={{ marginTop: 0 }}>{trade ? 'Edit trade' : 'Add trade'}</h3>

        <label style={{ display: 'block', marginTop: '0.75rem' }}>
          Account
          <select value={accountId} onChange={(e) => setAccountId(Number(e.target.value))} style={{ display: 'block', width: '100%', marginTop: '0.25rem' }}>
            {accounts.map((a) => (
        ...
        <div style={{ display: 'flex', gap: '0.75rem', marginTop: '0.75rem' }}>
          ...
          <label style={{ flex: 1 }}>
            Side
            <select value={side} onChange={(e) => setSide(e.target.value as 'long' | 'short')} style={{ display: 'block', width: '100%', marginTop: '0.25rem' }}>
