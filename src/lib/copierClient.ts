...
// Real trade copier, backed by eaglecapital-broker-sync's CopyFactory integration (MetaApi.cloud) —
// same backend/API URL as brokerSyncClient.ts. Mirrors deltaEngineClient.ts's exact contract
// (same type/field names) so TradeCopierPage.tsx needed near-zero changes to switch over.

const apiUrl = import.meta.env.VITE_BROKER_SYNC_API_URL

export const copierConfigured = Boolean(apiUrl)

if (!copierConfigured) {
  // eslint-disable-next-line no-console
  console.warn('Broker sync env var missing — set VITE_BROKER_SYNC_API_URL in .env.local. Trade Copier is disabled until then.')
}
...
export interface FollowerStats extends AccountStats {
  copierId: string
  multiplier: number
  riskMode: RiskMode
  isEnabled: boolean
...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (!res.ok) throw new Error(`Trade Copier API ${path} failed: ${res.status} ${await res.text()}`)
