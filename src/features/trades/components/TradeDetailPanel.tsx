...
const TABS: Tab[] = ['stats', 'strategy', 'tags', 'chart', 'notes']
    ...
    <div className={styles.panel}>
      <div className={styles.header}>
        <div className={styles.headerTop}>
          <button type="button" className={styles.navArrow} onClick={() => prevTrade && onSelect(prevTrade.id!)} disabled={!prevTrade}>‹</button>
          <span className={styles.symbol}>{trade.symbol}</span>
          <span className={styles.sideBadge}>{trade.side.toUpperCase()}</span>
          <span className={`${styles.outcomeBadge} ${OUTCOME_BADGE_CLASS[outcome]}`}>{OUTCOME_LABEL[outcome]}</span>
          <button type="button" className={styles.navArrow} onClick={() => nextTrade && onSelect(nextTrade.id!)} disabled={!nextTrade}>›</button>
        </div>
